...
```bash
{{ brand.executable_name }} copy-images [OPTIONS]
...
| `--destination`, `--dest` `DESTINATION`                               | `docker.io` (env: `{{ brand.env_prefix }}_SYSTEM_DEFAULT_CONTAINER_REGISTRY`) | Destination registry                                                    |
| `--destination-namespace`, `--dest-namespace` `DESTINATION_NAMESPACE` | (env: `{{ brand.env_prefix }}_RUNTIME_DEPLOY_DEFAULT_CONTAINER_NAMESPACE`)    | Destination namespace in the destination registry                       |
