...
# {{ brand.executable_name }} reload-config

Reload runtime-safe configuration values.

```bash
{{ brand.executable_name }} reload-config [OPTIONS]
```
...
| `--server-port` value               | `30080`                                | Target port of the {{ brand.name }} API server for applying or listing runtime config. When omitted, defaults to `{{ brand.env_prefix }}_API_PORT` if set, otherwise `30080`.                                                                                                                                                                                        |
| `--worker-port` value               | `10150`                                | Target port of the {{ brand.name }} worker for applying or listing runtime config. When omitted, defaults to `{{ brand.env_prefix }}_WORKER_PORT` if set, otherwise `10150`.                                                                                                                                                                                         |
