...
PACKAGE_NAMESPACE=${PACKAGE_NAMESPACE:-6block}
PACKAGE_REPOSITORY=${PACKAGE_REPOSITORY:-llmfabric}
...
function pack() {
    ...
    LABELS=("org.opencontainers.image.source=https://github.com/6block/llmfabric" "org.opencontainers.image.version=main" "org.opencontainers.image.revision=$(git rev-parse HEAD 2>/dev/null || echo "unknown")" "org.opencontainers.image.created=$(date +"%Y-%m-%dT%H:%M:%S.%s")");
    TAG="${PACKAGE_NAMESPACE}/${PACKAGE_REPOSITORY}:${PACKAGE_TAG}"
    EXTRA_ARGS=()
	if [[ "${PACKAGE_WITH_CACHE}" == "true" ]]; then
		EXTRA_ARGS+=("--cache-from=type=registry,ref=6block/llmfabric:main")
	fi
